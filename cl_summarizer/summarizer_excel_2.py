...
def df_sales_data(cursor,i):
    ...
    df_excel.applymap(lambda x: x.strip() if isinstance(x, str) else x)
...
def fill_summary_sheet(supp_desc,summary_index_list,path_export_final,vendor_num):
    with xw.App(visible=False) as app:
        ...
        range_length_to_remove = str(length_start -1)+':'+ str(68)
...
def main():
    ...
    for i in range(1,count_sheets_excel_file+1):
        print('-----------------')
        print(f'sheet {i}')
        df_merge,cd_ref,link_dict = df_sales_data(cursor,i)
        # print(df_merge)
        # df_Sales
        ...
        if key == 'LESSTHAN20K':
            ...
            for key,value in dict_loc_insert_more.items():
                ...
                try:
                    ...
                    pass
                i_insert_2+=2
            move_worksheet_to_vba_template(path_xlsb = path_export_final_morethan20k)
