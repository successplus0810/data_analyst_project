with temp as
(
SELECT * FROM TABLE(COLES.STI_WIP_CS.SUM_CS_MISSING_{}_VENDOR (STRTOK_TO_ARRAY('{}',','),DATE('{}'::DATE,'YYYY-MM-DD'),DATE('{}'::DATE,'YYYY-MM-DD'),round({},2)::FLOAT,{}::varchar))
)
...
round(rtotal_sales_amt_mod_promo,2) as rtotal_sales_amt_mod_promo, rqty_promo, round(rf_actual_sell_price_amt_mod_1,2) as rf_actual_sell_price_amt_mod_1, round({},2) AS SCAN_RATE
from temp
where RQTY_PROMO != 0
