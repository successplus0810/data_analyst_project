...
def item_gst(i):
    ...
    for key,value in item_list_dict.items():
        if gst == 10:
            ...
            pass   
    return supp_num,supp_desc,vendor_num,claim_number,gst,clm_start,clm_end,dept,item_unique,item_list_dict,classify_state,file_path_excel,file_path_email
...
def df_sales_data(item_list_dict_gsted,classify_state):
    i = 0
    ...
    if classify_state.lower() == 'state':
        for key,value in item_list_dict_gsted.items():
            print(key,value)
            item_code,state = key
            df_each_item = connect_sql(cursor,file_sql = file_sql_summ_state ,item_code = item_code,start_date = clm_start,end_date =clm_end,var_1=value[0],var_2=value[1],var_3 = state)
            if i == 0:
...
for i in range(1,count_sheets_excel_file+1):
    print(f'Sheet {i}')
    supp_num,supp_desc,vendor_num,claim_number,gst,clm_start,clm_end,dept,item_unique,item_list_dict,classify_state,file_path_excel,file_path_email = item_gst(i)
    df_sales = df_sales_data(item_list_dict,classify_state)
