...
folder_name = '202203'
month_filter = '202203'
dna_check = ['START TO REVIEW 11/2023','QUERY CM','']
###############################################
...
def main():
    df_raw_check = pd.DataFrame()
    cursor = set_up(config = config_coles)
    df_raw = connect_sql(cursor = cursor,file_sql = file_sql_claimpack,item_code=month_filter)
    ...
    for supp_num,list_pmt_id in dict_sup_pro.items():
        ...
        for pmt_id in list_pmt_id:
            ...
            #     df_splited['CHECK_COLUMN'] = 'ELI3 <= 0'
                ...
                clm_start_check_converted = datetime.datetime.strptime(clm_start_check,'%Y-%m-%dT%H:%M:%S.000%f')
                # print(clm_end_check_converted)
                clm_start_check_converted_2 = int(datetime.datetime.timestamp(clm_start_check_converted)) * 10**9
                df_cd_check_cole_online = connect_sql(cursor = cursor,file_sql = file_sql_cd_check_cole_online,item_code=item_list_final)
                df_cd_check_cole_online_cols = df_cd_check_cole_online[df_cd_check_cole_online['CLM_DESC'].str.contains('COLS')]
                ...
                if df_splited['PRMTN_TYPE_CDE'].unique()[0] == 'TH':
                    ...
                    if df_cd_check_cole_online_cols.empty:
                        flag_co = True
                    else:
                        df_list = df_cd_check_cole_online_cols[['CLM_START','CLM_END']].values.tolist()
                        flag_co = True
                        for sub_list in df_list:
                            if (clm_end_check_converted_2 >= sub_list[0] and clm_end_check_converted_2 <= sub_list[1]) or (clm_start_check_converted_2 >= sub_list[0] and clm_start_check_converted_2 <= sub_list[1]):
                                flag_co = False
                            else:
                                pass
                #################
            ...
            #  Check DNA :
            dna_action = df_splited['DNA_ACTION'].unique()[0]
            check_column = df_splited['CHECK_COLUMN'].unique()[0]
            if dna_action == None:
                dna_action = ''
            if dna_action in dna_check :
                print(check_column + dna_action)
                df_splited['CHECK_COLUMN'] = check_column + '_' +dna_action
            else:
                df_splited['CHECK_COLUMN'] = dna_action
            if j  == 0:
