...
def set_up(config):
    ...
    cursor = conn.cursor()
    return cursor
def connect_sql(cursor,file_sql,item_code=0,var_1=0,var_2=0,var_3=0,var_4=0,var_5=0):
    try:
...
def df_sales_data(cursor, item_list_dict_gsted,file_sql,start_date,end_date,pmt_id_temp):
    i = 0
    for key,value in item_list_dict_gsted.items():
        df_each_item = connect_sql(cursor = cursor,file_sql = file_sql ,item_code = key,var_1 =pmt_id_temp , var_2 = start_date,var_3=end_date,var_4=value[0],var_5=value[1])
        if i == 0:
...
def product_state_summary(df,df_ref):
    ...
    df_final = product_state_df(df_sales = df_temp, df_ref=df_ref)
...
def product_state_df(df_sales,df_ref):
    ...
    if df_ref.empty == False:
        ...
        print('df_ref_groupby_qty',df_ref_groupby_qty)
        # df_ref_groupby_qty['ITEM_IDNT'] = df_ref_groupby_qty['ITEM_IDNT'].astype(int64)
...
def product_summary(df):
    ...
    dict_data = {'df':df_product,'cell_export':'B20'}
...
def cd_ref(prmt_id,cursor,file_sql,item_code,df_sales):
    print('Start cd ref')
    list_data = []
    list_remove = []
    ...
    list_data.append(dict_data)
    list_remove.append(dict_remove)
    return df_ref,df_sales_daily,list_data,list_remove
...
def main():
    ...
    for supp_num,list_pmt_id in dict_sup_pro.items():
        ...
        for pmt_id in list_pmt_id:
            ...
            if math.isnan(float(supp_num)):
                df_splited['CHECK_COLUMN'] = 'NULL SUPPLIER'
            else:
                if (float(supp_num) in list_dna) or math.isnan(float(supp_num)):
    ...
    df_raw_filter = df_raw_check[(df_raw_check['CHECK_COLUMN'] == 'TO QA')] 
    ...
    for supp_num,list_pmt_id in dict_sup_pro_filter.items():
        ...
        for pmt_id in list_pmt_id:
            ...
            clm_start = df_splited_filter['CLM_START'].unique()[0]
            clm_end = df_splited_filter['CLM_END'].unique()[0]
            ...
            if df_splited_filter[['PRM_PRICE','CLM_RATE']].drop_duplicates().count()['PRM_PRICE'] == 1 :
                ...
                df_splited_filter_2 = df_splited_filter[['SKU_ID','PRM_PRICE','CLM_RATE']]
                df_splited_filter_2= df_splited_filter_2.drop_duplicates()
                print(df_splited_filter_2)
                item_list_dict = df_splited_filter_2.set_index('SKU_ID')[['PRM_PRICE','CLM_RATE']].to_dict('index')
            ...
            pmt_temp = pmt_id
            df_sales = df_sales_data(cursor = cursor,item_list_dict_gsted = item_list_dict,file_sql = file_sql_summ ,start_date = clm_start,end_date = clm_end, pmt_id_temp = pmt_temp)
            df_ref,df_sales,list_data_sales,list_remove_sales  = cd_ref(prmt_id=pmt_temp,cursor = cursor,file_sql=file_sql_cd_ref,item_code=item_list_final,df_sales=df_sales)  
